...
def test_get_first_last():
    ...
    test3 = "DEERE DOE, JOHN C III USAF SQUADRON UNIT/FLIGHT <email.3.4@us.af.mil>;"
...
def test_remove_semic_newline():
    line = "THOMAS SPENCER, JAMES LEE II TSgt USAF AFDW BAND/BABC <james.l.thomas-spencer@us.af.mil>;\n"
...
def test_is_distro_name():
    ...
    not_distro3 = "THOMAS SPENCER, JAMES LEE II TSgt USAF AFDW BAND/BABC <james.l.thomas-spencer@us.af.mil>;\n"
    assert parse_roster.is_distro_name(distro) is True
    assert parse_roster.is_distro_name(not_distro) is False
    assert parse_roster.is_distro_name(not_distro2) is False
    assert parse_roster.is_distro_name(not_distro3) is False
...
def test_to_remove_from_distro_list():
    ...
    to_remove = ["\n--TEST_LIST--", f"CHRISTOPHER STEVENS {correct_email}"]

    assert (
        parse_roster.to_remove_from_distro_list(
            TEST_DISTRO_CORRECT, parse_roster.load_roster(TEST_ROSTER_CORRECT)
        )
        == to_remove
    )
